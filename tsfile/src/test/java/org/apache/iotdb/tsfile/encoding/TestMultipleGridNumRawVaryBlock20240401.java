...
public class TestMultipleGridNumRawVaryBlock20240401 {
  ...
  public static byte[] bitPacking_1d(ArrayList<Integer> numbers, int bit_width) {
    int block_num = numbers.size() / 8;
    byte[] result = new byte[bit_width * block_num];
    for (int i = 0; i < block_num; i++) {
      for (int j = 0; j < bit_width; j++) {
  ...
  public static int getEncodeBits(
      ArrayList<ArrayList<Integer>> ts_block,
      int block_size,
      int grid,
      ArrayList<Integer> result,
      ArrayList<Integer> value_block,
      ArrayList<Integer> time_r_block,
      ArrayList<Integer> outlier_val_block,
      ArrayList<Integer> outlier_pos,
      ArrayList<Integer> gridnum_block,
      ArrayList<Integer> gridpos,
      boolean flag_use_anomaly,
      double para_k) {

    int value_delta_min = Integer.MAX_VALUE;

    ArrayList<Integer> value_list = new ArrayList<>();
    ArrayList<Integer> time_r_list = new ArrayList<>();
    ArrayList<Integer> time_grid_list = new ArrayList<>();
...
//      int epsilon_r = ts_block.get(j).get(0) - ts_block.get(0).get(0) - gridNum_r * grid;
      ...
      value_list.add(epsilon_v);
      time_r_list.add(epsilon_r);

      //使用前后做差的思路
      time_grid_list.add(gridNum_r);
//      //使用格点的思路2
//      time_grid_list.add(gridNum_r - pre_gridNum_r);
//      pre_gridNum_r = gridNum_r;
    ...
    for (int j = 0; j < time_grid_list.size(); j++) {
      if (time_grid_list.get(j) != 1) {
        gridnum_block.add(time_grid_list.get(j));
        gridpos.add(1);
        if (time_grid_list.get(j) > max_gridnum_val) {
          max_gridnum_val = time_grid_list.get(j);
        }
      }
      else {
        gridpos.add(0);
    ...
    for (int j = 0; j < value_list.size(); j++){
      int v = value_list.get(j) - value_delta_min;
      value_block.add(v);
      if(v > max_v){
        max_v = v;
      }
    ...
    int time_r_gr = 1;
    ArrayList<Integer> med_block = new ArrayList<>(time_r_list);
    Collections.sort(med_block);
    time_r_gr = med_block.get(med_block.size() / 2);

    int sum=0;
    int dsum = 0;
    for (int j = 0; j < time_r_list.size(); j++) {
      sum += time_r_list.get(j);
      dsum += time_r_list.get(j) * time_r_list.get(j);
    }
    int time_r_avg = (int) (sum * 1.0 / time_r_list.size());
    int time_r_std = (int) Math.sqrt(dsum * 1.0 / time_r_list.size());

//    if(time_r_std==0){
//      flag_use_anomaly=false; //按照正态分布检测时,若标准差为0,则放弃异常检测
//    }

    if(grid<20){
      flag_use_anomaly=false; //按照grid检测时,若grid本身太小,则放弃异常检测
    }

    int r_min = Integer.MAX_VALUE;
    int outlier_val_min = Integer.MAX_VALUE;
    if(flag_use_anomaly) {
      for (int j = 0; j < time_r_list.size(); j++) {
        //if (Math.abs(time_r_list.get(j) - time_r_avg) > 3 * time_r_std) {
        if (Math.abs(time_r_list.get(j) - time_r_gr) > para_k * grid) {
          System.out.println(time_r_list.get(j)+"-"+time_r_gr+"-"+grid);
          //System.out.println(time_r_list.get(j)+"-"+time_r_gr+"-"+time_r_std);
          int outlier = time_r_list.get(j);
          outlier_val_block.add(outlier);
          if (outlier < outlier_val_min) {
            outlier_val_min = outlier;
          }
        } else {
          outlier_pos.add(0);
          time_r_block.add(time_r_list.get(j));
          if (time_r_list.get(j) < r_min) {
            r_min = time_r_list.get(j);
          }
        }
      }
    }
    ...
    int max_time_r = Integer.MIN_VALUE;
    for (int j = 0; j < time_r_block.size(); j++) {
      int t = time_r_block.get(j) - r_min;
      time_r_block.set(j, t);
      if(t > max_time_r){
        max_time_r = t;
      }
    }

    int max_outlier_val = Integer.MIN_VALUE;
    if(flag_use_anomaly) {
      for (int j = 0; j < outlier_val_block.size(); j++) {
        int outlier = outlier_val_block.get(j) - outlier_val_min;
        outlier_val_block.set(j, outlier);
        if (outlier > max_outlier_val) {
          max_outlier_val = outlier;
        }
      }
    }
    else{
    ...
    int max_bit_width_value = getBitWith(max_v);
    int max_bit_width_time_r = getBitWith(max_time_r);
    int max_bit_width_outlier_val = getBitWith(max_outlier_val);
    int max_bit_width_gridnum_val = getBitWith(max_gridnum_val);

    int length = max_bit_width_value * value_block.size() //value
            + max_bit_width_time_r * time_r_block.size() //time_r_block
            + max_bit_width_outlier_val * outlier_val_block.size() //outlier_val_block
            + (block_size - 1) //outlier_pos
            + (max_bit_width_gridnum_val + 1) * (timestamp_gridnum_length + timestamp_gridnum_remain_length);
    result.clear();

    result.add(length);
    result.add(max_bit_width_value);
    result.add(max_bit_width_time_r);
    result.add(max_bit_width_outlier_val);
    result.add(max_bit_width_gridnum_val);

    result.add(value_delta_min);
    result.add(r_min);
    result.add(outlier_val_min);

    result.add(ts_block.get(0).get(1)); //value0
    result.add(ts_block.get(0).get(0)); //time0
    result.add(grid);
  ...
  public static ArrayList<Byte> encode2BytesOrigin(
      ArrayList<ArrayList<Integer>> ts_block,
    ...
    byte[] gridnum_val_bytes = bitPacking_1d(gridnum_block, raw_length.get(3));
    for (byte b : gridnum_val_bytes) encoded_result.add(b);

    byte[] gridnum_pos_bytes = bitPacking_1d(gridpos, 1);
  ...
  public static ArrayList<Byte> encode2Bytes(
    ...
    // encode time0 and value0
    byte[] value0_byte = int2Bytes(raw_length.get(8)); //v0
    for (byte b : value0_byte) encoded_result.add(b);
    byte[] time0_byte = int2Bytes(raw_length.get(9)); //t0
    for (byte b : time0_byte) encoded_result.add(b);

    // encode value_min, r_min, outlier_val_min
    byte[] value_min_byte = int2Bytes(raw_length.get(5));
    for (byte b : value_min_byte) encoded_result.add(b);
    byte[] r_min_byte = int2Bytes(raw_length.get(6));
    for (byte b : r_min_byte) encoded_result.add(b);
    byte[] outlier_val_min_byte = int2Bytes(raw_length.get(7));
    for (byte b : outlier_val_min_byte) encoded_result.add(b);

    // encode value_block
    byte[] max_bit_width_value_byte = int2Bytes(raw_length.get(1));
    for (byte b : max_bit_width_value_byte) encoded_result.add(b);
    byte[] value_bytes = bitPacking_1d(value_block, raw_length.get(1));
    for (byte b : value_bytes) encoded_result.add(b);



    // encode time_r_block
    byte[] max_bit_width_time_r_byte = int2Bytes(raw_length.get(2));
    for (byte b : max_bit_width_time_r_byte) encoded_result.add(b);
    byte[] timestamp_bytes = bitPacking_1d(time_r_block, raw_length.get(2));
    for (byte b : timestamp_bytes) encoded_result.add(b);

    // encode outlier_val_block
    byte[] max_bit_width_outlier_val_byte = int2Bytes(raw_length.get(3));
    for (byte b : max_bit_width_outlier_val_byte) encoded_result.add(b);
    byte[] timestamp_outlier_bytes = bitPacking_1d(outlier_val_block, raw_length.get(3));
    for (byte b : timestamp_outlier_bytes) encoded_result.add(b);

    // encode outlier_pos_block
    byte[] outlier_pos_bytes = bitPacking_1d(outlier_pos, 1);
    for (byte b : outlier_pos_bytes) encoded_result.add(b);

    // encode gridnum_value
    byte[] max_bit_width_gridnum_value_byte = int2Bytes(raw_length.get(4));
    for (byte b : max_bit_width_gridnum_value_byte) encoded_result.add(b);
    byte[] gridnum_val_bytes = bitPacking_1d(gridnum_block, raw_length.get(4));
    for (byte b : gridnum_val_bytes) encoded_result.add(b);

    byte[] gridnum_pos_bytes = bitPacking_1d(gridpos, 1);
    for (byte b : gridnum_pos_bytes) encoded_result.add(b);

    byte[] grid_byte = int2Bytes(raw_length.get(10));
    for (byte b : grid_byte) encoded_result.add(b);

    return encoded_result;
  }

  public static ArrayList<Byte> encode2BytesTs2diff(
  ...
  public static ArrayList<Byte> ReorderingRegressionEncoder(
      ArrayList<ArrayList<Integer>> data, int block_size, double para_k) {
    block_size++;
    ArrayList<Byte> encoded_result = new ArrayList<>();
    ...
    for (int i = 0; i < block_num; i++) {
      ...
      ArrayList<Integer> value_block = new ArrayList<>();
      ArrayList<Integer> time_r_block = new ArrayList<>();
      ArrayList<Integer> outlier_val_block = new ArrayList<>();
      ArrayList<Integer> outlier_pos = new ArrayList<>();
      ...
      ratio = 1.0;
      if (ratio > 0.2) {
        getEncodeBits(ts_block, block_size, grid,raw_length,
           value_block, time_r_block, outlier_val_block,outlier_pos, gridnum_block, gridpos,false, para_k);
        cur_encoded_result = encode2Bytes(raw_length, grid, value_block, time_r_block,
                outlier_val_block,outlier_pos,gridnum_block, gridpos);
      }
      else {
        ArrayList<ArrayList<Integer>> ts_block_delta2 =
                getEncodeBitsTs2diff(ts_block, block_size, raw_length2);
        cur_encoded_result = encode2BytesTs2diff(ts_block_delta2, raw_length2);
      }
...
//
//      getEncodeBits(ts_block, remaining_length, grid,
//              raw_length, value_block, time_r_block, outlier_val_block,outlier_pos, gridnum_block, gridpos);
//
//      ArrayList<ArrayList<Integer>> ts_block_delta2 =
...
//    }
  ...
  public static void main(String[] args) throws IOException {
    ...
    input_path_list.add("E:\\thu\\TimeEncoding\\TestTimeGrid\\result_python\\iotdb_test\\TY-Fuel"); //1
    output_path_list.add("E:\\thu\\TimeEncoding\\TestTimeGrid\\result_python\\result_evaluation\\compression_ratio\\vary_block_size\\TY-Fuel_ratio.csv");
    input_path_list.add("E:\\thu\\TimeEncoding\\TestTimeGrid\\result_python\\iotdb_test\\Metro-Traffic"); //2
    output_path_list.add("E:\\thu\\TimeEncoding\\TestTimeGrid\\result_python\\result_evaluation\\compression_ratio\\vary_block_size\\Metro-Traffic_ratio.csv");
    input_path_list.add("E:\\thu\\TimeEncoding\\TestTimeGrid\\result_python\\iotdb_test\\Temperature"); //3
    output_path_list.add("E:\\thu\\TimeEncoding\\TestTimeGrid\\result_python\\result_evaluation\\compression_ratio\\vary_block_size\\Temperature_ratio.csv");
    input_path_list.add("E:\\thu\\TimeEncoding\\TestTimeGrid\\result_python\\iotdb_test\\Humidity"); //4
    output_path_list.add("E:\\thu\\TimeEncoding\\TestTimeGrid\\result_python\\result_evaluation\\compression_ratio\\vary_block_size\\Humidity_ratio.csv");
...
//
//    input_path_list.add("E:\\thu\\TimeEncoding\\TestTimeGrid\\result_python\\iotdb_test\\air_quality"); //8
//    output_path_list.add("E:\\thu\\TimeEncoding\\TestTimeGrid\\result_python\\result_evaluation\\compression_ratio\\rr_ratio\\air_quality_ratio.csv");
//    input_path_list.add("E:\\thu\\TimeEncoding\\TestTimeGrid\\result_python\\iotdb_test\\dirty_energy"); //8
//    output_path_list.add("E:\\thu\\TimeEncoding\\TestTimeGrid\\result_python\\result_evaluation\\compression_ratio\\rr_ratio\\dirty_energy_ratio.csv");
//    input_path_list.add("E:\\thu\\TimeEncoding\\TestTimeGrid\\result_python\\iotdb_test\\energy"); //8
//    output_path_list.add("E:\\thu\\TimeEncoding\\TestTimeGrid\\result_python\\result_evaluation\\compression_ratio\\rr_ratio\\energy_ratio.csv");
//    input_path_list.add("E:\\thu\\TimeEncoding\\TestTimeGrid\\result_python\\iotdb_test\\pm"); //8
//    output_path_list.add("E:\\thu\\TimeEncoding\\TestTimeGrid\\result_python\\result_evaluation\\compression_ratio\\rr_ratio\\pm_ratio.csv");
    ...
    for (int file_i = 0; file_i < input_path_list.size(); file_i++) {
      ...
      for (File f : tempList) {
        //for (int vary_p = 1; vary_p < 5; vary_p++) {
        for (int vary_p = 4; vary_p < 9; vary_p++) {
          ...
          for (int i = 0; i < repeatTime; i++) {
            ...
            for (int repeat = 0; repeat < repeatTime2; repeat++) {
              //buffer = ReorderingRegressionEncoder(data, (int) Math.pow(2, 8), 0.05 * vary_p);
              buffer = ReorderingRegressionEncoder(data, (int) Math.pow(2, vary_p), 0.1);
            }
